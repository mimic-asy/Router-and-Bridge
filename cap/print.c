...
// MAC アドレスを文字列化する。多重呼び出し用に格納先バッファも引数で渡す
...
// ARPヘッダで扱われているu_int8_t型で配列されているIPアドレスを文字列化する
...
// iphdrではu_int32tでipアドレスを扱うため、u_int32t->structin_addr->文字列に変換する関数
char *ip_ip2str(u_int32_t ip, char *buf, socklen_t size)
{
    struct in_addr *addr;

    addr = (struct in_addr *)&ip;
    // inet_ntopはstruct in_addrを文字列化する
...
int PrintEtherHeader(struct ether_header *eh, FILE *fp)
{
    ...
    fprintf(fp, "ether_dhost=%s¥n", my_ether_ntoa_r(eh->ether_dhost, buf, sizeof(buf)));
